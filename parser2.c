...
void parser_valueToken(char* text, JSON* json, int* pIdx, int* pNoT) {
	// remove space
	for (; text[*pIdx] == 32 || text[*pIdx] == 10 || text[*pIdx] == 13; (*pIdx)++);
...
void parser_valueToken_ARRAY(char* text, JSON* json, int* pIdx, int* pNoT) {
	...
	//printf("Array [%d:%d] size:%d\n", json->tokens[*pNoT-1].start, json->tokens[*pNoT-1].end, json->tokens[*pNoT-1].stringLength);
	//printf("Array C: %s\n\n", json->tokens[*pNoT-1].value);
	//printf("<s %c> , <e %c>\n", text[json->tokens[*pNoT-1].start], text[json->tokens[*pNoT-1].end]);

	while (*pIdx < tempIdx) {

		// remove space
		for (; text[*pIdx] == 32 || text[*pIdx] == 13 || text[*pIdx] == ',' || text[*pIdx] == 10; (*pIdx)++);
		//printf("test\n");
		//printf("(idx: %d) (prec: %d, c: %d, postc: %d)\n", (*pIdx), (text[*pIdx-1]), (text[*pIdx]), (text[*pIdx+1]));		
		//getchar();
		parser_valueToken(text, json, pIdx, pNoT);
	}
